...
def before_all() -> None:
	...
	[
		...
		'https://github.com/educationvdo/educationvdo-assets/releases/download/examples-3.0.0/source.mp3',
...
def test_sync_lip_to_image(workflow_strategy : WorkflowStrategy) -> None:
	commands = [ sys.executable, 'educationvdo.py', 'headless-run', '--jobs-path', get_test_jobs_directory(), '--workflow-strategy', workflow_strategy, '--processors', 'lip_syncer', '-s', get_test_example_file('source.mp3'), '-t', get_test_example_file('target-240p.jpg'), '-o', get_test_output_file('test_sync_lip_to_image.jpg') ]
...
def test_sync_lip_to_video(workflow_strategy : WorkflowStrategy) -> None:
	commands = [ sys.executable, 'educationvdo.py', 'headless-run', '--jobs-path', get_test_jobs_directory(), '--workflow-strategy', workflow_strategy, '--processors', 'lip_syncer', '-s', get_test_example_file('source.mp3'), '-t', get_test_example_file('target-240p.mp4'), '-o', get_test_output_file('test_sync_lip_to_video.mp4'), '--trim-frame-end', '1' ]
