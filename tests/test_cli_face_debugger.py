...
def test_debug_face_to_image(workflow_strategy : WorkflowStrategy) -> None:
	commands = [ sys.executable, 'educationvdo.py', 'headless-run', '--jobs-path', get_test_jobs_directory(), '--workflow-strategy', workflow_strategy, '--processors', 'face_debugger', '-t', get_test_example_file('target-240p.jpg'), '-o', get_test_output_file('test-debug-face-to-image.jpg') ]
...
def test_debug_face_to_video(workflow_strategy : WorkflowStrategy) -> None:
	commands = [ sys.executable, 'educationvdo.py', 'headless-run', '--jobs-path', get_test_jobs_directory(), '--workflow-strategy', workflow_strategy, '--processors', 'face_debugger', '-t', get_test_example_file('target-240p.mp4'), '-o', get_test_output_file('test-debug-face-to-video.mp4'), '--trim-frame-end', '1' ]
