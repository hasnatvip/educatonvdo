...
from educationvdo import content_analyser, state_manager
from educationvdo.execution import resolve_cache_path
from educationvdo.inference_manager import get_inference_pool, resolve_static_inference_providers
...
def test_get_inference_pool() -> None:
	...
	with patch('educationvdo.inference_manager.has_execution_provider', return_value = True):
		with patch('educationvdo.inference_manager.get_onnxruntime_version', return_value = (1, 26, 0)):

			with patch('educationvdo.inference_manager.detect_app_context', return_value = 'cli'):
				cli_inference_pool = get_inference_pool('educationvdo.content_analyser', model_names, model_source_set)

				assert isinstance(cli_inference_pool.get('nsfw_1'), InferenceSession)

			with patch('educationvdo.inference_manager.detect_app_context', return_value = 'ui'):
				ui_inference_pool = get_inference_pool('educationvdo.content_analyser', model_names, model_source_set)
	...
	with patch('educationvdo.inference_manager.get_onnxruntime_version', return_value = (1, 24, 4)):
...
def test_resolve_static_inference_providers(override_module : SimpleNamespace, adjust_module : SimpleNamespace) -> None:
	...
	with patch('educationvdo.inference_manager.importlib', Mock(import_module = Mock(return_value = override_module))):
	...
	with patch('educationvdo.inference_manager.importlib', Mock(import_module = Mock(return_value = adjust_module))):
