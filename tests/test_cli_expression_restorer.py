...
def test_restore_expression_to_image(workflow_strategy : WorkflowStrategy) -> None:
	commands = [ sys.executable, 'educationvdo.py', 'headless-run', '--jobs-path', get_test_jobs_directory(), '--workflow-strategy', workflow_strategy, '--processors', 'expression_restorer', '-t', get_test_example_file('target-240p.jpg'), '-o', get_test_output_file('test-restore-expression-to-image.jpg') ]
...
def test_restore_expression_to_video(workflow_strategy : WorkflowStrategy) -> None:
	commands = [ sys.executable, 'educationvdo.py', 'headless-run', '--jobs-path', get_test_jobs_directory(), '--workflow-strategy', workflow_strategy, '--processors', 'expression_restorer', '-t', get_test_example_file('target-240p.mp4'), '-o', get_test_output_file('test-restore-expression-to-video.mp4'), '--trim-frame-end', '1' ]
