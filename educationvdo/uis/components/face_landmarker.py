...
from educationvdo import face_landmarker, state_manager, translator
from educationvdo.common_helper import calculate_float_step
from educationvdo.types import FaceLandmarkerModel, Score
...
def render() -> None:
	...
	FACE_LANDMARKER_MODEL_DROPDOWN = gradio.Dropdown(
		...
		choices = educationvdo.choices.face_landmarker_models,
	...
	FACE_LANDMARKER_SCORE_SLIDER = gradio.Slider(
		...
		step = calculate_float_step(educationvdo.choices.face_landmarker_score_range),
		minimum = educationvdo.choices.face_landmarker_score_range[0],
		maximum = educationvdo.choices.face_landmarker_score_range[-1]
	)
