...
from educationvdo.types import BenchmarkMode, BenchmarkResolution
...
def render() -> None:
	...
	BENCHMARK_MODE_DROPDOWN = gradio.Dropdown(
		...
		choices = educationvdo.choices.benchmark_modes,
	...
	BENCHMARK_RESOLUTIONS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = educationvdo.choices.benchmark_resolutions,
	...
	BENCHMARK_CYCLE_COUNT_SLIDER = gradio.Slider(
		...
		step = calculate_int_step(educationvdo.choices.benchmark_cycle_count_range),
		minimum = educationvdo.choices.benchmark_cycle_count_range[0],
		maximum = educationvdo.choices.benchmark_cycle_count_range[-1]
	)
