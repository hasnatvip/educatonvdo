...
from educationvdo import config, content_analyser, inference_manager, logger, state_manager, translator, video_manager, voice_extractor
from educationvdo.common_helper import create_int_metavar, is_macos
...
from educationvdo.processors.modules.frame_colorizer import choices as frame_colorizer_choices
from educationvdo.processors.modules.frame_colorizer.types import FrameColorizerInputs
...
from educationvdo.types import ApplyStateItem, Args, DownloadScope, InferencePool, InferenceProvider, ModelOptions, ModelSet, ProcessMode, VisionFrame
from educationvdo.vision import blend_frame, read_static_image, read_static_video_frame, unpack_resolution
...
def override_inference_providers() -> List[InferenceProvider]:
	if is_macos() and has_execution_provider('coreml'):
		return [ educationvdo.choices.execution_provider_set.get('cpu') ]
...
def register_args(program : ArgumentParser) -> None:
	...
	if group_processors:
		...
		educationvdo.jobs.job_store.register_step_keys([ 'frame_colorizer_model', 'frame_colorizer_blend', 'frame_colorizer_size' ])
...
def get_common_modules() -> List[ModuleType]:
	return [ content_analyser, voice_extractor ]
