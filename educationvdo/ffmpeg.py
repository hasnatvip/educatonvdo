...
def get_available_encoder_set() -> EncoderSet:
	...
	while line := process.stdout.readline().decode().lower():
		if line.startswith(' a'):
			...
			if audio_encoder in educationvdo.choices.output_audio_encoders:
				index = educationvdo.choices.output_audio_encoders.index(audio_encoder) #type:ignore[arg-type]
				available_encoder_set['audio'].insert(index, audio_encoder) #type:ignore[arg-type]
		if line.startswith(' v'):
			video_encoder = line.split()[1]

			if video_encoder in educationvdo.choices.output_video_encoders:
				index = educationvdo.choices.output_video_encoders.index(video_encoder) #type:ignore[arg-type]
