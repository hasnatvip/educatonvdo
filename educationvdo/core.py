...
from educationvdo import benchmarker, cli_helper, logger, state_manager, translator
...
from educationvdo.program import create_program
from educationvdo.program_helper import validate_args
from educationvdo.types import Args, ErrorCode, WorkflowMode
from educationvdo.workflows import image_to_image, image_to_video
...
def pre_check() -> bool:
	if sys.version_info < (3, 10):
		logger.error(
			translator.get('python_not_supported').format(version='3.10'),
			__name__
		)
		return False
	...
	for dependency in ['curl', 'ffmpeg', 'ffprobe']:
		if not shutil.which(dependency):
			logger.error(
				translator.get('dependency_not_installed').format(
...
def common_pre_check() -> bool:
	"""
	NSFW/content analyser integrity check disabled.

	The content_analyser module is no longer required
	for the application pre-check.
	"""
	return True
...
def force_download() -> ErrorCode:
	...
	available_processors = [
		...
		for file_path in resolve_file_paths('educationvdo/processors/modules')
	]
		...
		if hasattr(module, 'create_static_model_set'):
			for model in module.create_static_model_set(download_scope).values():
				...
				if model_hash_set and model_source_set:
					if (
						not conditional_download_hashes(model_hash_set)
...
def route_job_manager(args: Args) -> ErrorCode:
	...
	if state_manager.get_item('command') == 'job-submit-all':
		if job_manager.submit_jobs(
			state_manager.get_item('halt_on_error')
		):
			logger.info(
				translator.get('job_all_submitted'),
				__name__
			)
			return 0

		logger.error(
			translator.get('job_all_not_submitted'),
			__name__
		)
		return 1
	...
	if state_manager.get_item('command') == 'job-delete-all':
		if job_manager.delete_jobs(
			...
			logger.info(
				translator.get('job_all_deleted'),
				__name__
			)
			return 0

		logger.error(
			translator.get('job_all_not_deleted'),
			__name__
		)
		return 1
...
def route_job_runner() -> ErrorCode:
	...
	if state_manager.get_item('command') == 'job-run-all':
		logger.info(
			translator.get('running_jobs'),
			__name__
		)

		if job_runner.run_jobs(
			process_step,
			state_manager.get_item('halt_on_error')
		):
			logger.info(
				translator.get('processing_jobs_succeeded'),
				__name__
			)
			return 0

		logger.info(
			translator.get('processing_jobs_failed'),
			__name__
		)
		return 1
	...
	if state_manager.get_item('command') == 'job-retry-all':
		logger.info(
			translator.get('retrying_jobs'),
			__name__
		)

		if job_runner.retry_jobs(
			process_step,
			state_manager.get_item('halt_on_error')
		):
			logger.info(
...
def process_batch(args: Args) -> ErrorCode:
	...
	source_paths = resolve_file_pattern(
		job_args.get('source_pattern')
	)
	target_paths = resolve_file_pattern(
		job_args.get('target_pattern')
	)
	...
	if job_manager.create_job(job_id):
		if source_paths and target_paths:
			for index, (source_path, target_path) in enumerate(
				...
				try:
					step_args['output_path'] = job_args.get(
						'output_pattern'
					).format(
						index=index,
